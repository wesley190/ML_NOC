...
SC_MODULE( Controller ) {
    ...
    void transmit_to_router(){
        if(ready_tx.read() == 1 && valid_tx == 1){
            ...
            if(tx_dara_cnt == tx_total_amount-1){
                rx_dara_cnt = 0;
                tx_dara_cnt = 0;
                tx_cnt = 0;
                layer_tmp++;
        ...
        if(rx_dara_cnt != 0){
            if(tx_cnt == 0){ // Begin of packet
                ...
                valid_tx.write(1);
                if((tx_cnt == input_size && layer_tmp%2 == 0 && layer_tmp != 0) || tx_dara_cnt == (tx_total_amount-1)){ // End of Packet
