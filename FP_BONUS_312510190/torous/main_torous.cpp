...
#include "core_torous.h"
//#include "pe.h"
#include "router_torous.h"
#include "controller_torous.h"
#include "ROM.h"
...
int sc_main(int argc, char* argv[])
{
    ...
    // to router0
    sc_signal < sc_lv<34> > flit_tx_con;
    sc_signal < bool > valid_tx_con;
    sc_signal  < bool > ready_tx_con;
    ...
    Controller* m_Controller = new Controller("m_Controller");
    
    ROM m_ROM("m_ROM");


    Core* m_Core0 = new Core("m_Core0", 0);
    Core* m_Core1 = new Core("m_Core1", 1);
    Core* m_Core2 = new Core("m_Cor2", 2);
    Core* m_Core4 = new Core("m_Core4", 4);
    ...
    m_Controller->flit_tx( flit_tx_con );
    m_Controller->valid_tx( valid_tx_con );
    m_Controller->ready_tx( ready_tx_con );
    m_Controller->flit_rx( out_flit[4][2] );
    m_Controller->valid_rx( out_valid[4][2] );
    m_Controller->ready_rx( out_ready[4][2] );
    ...
    //m_Controller(rst, clk, layer_id, layer_id_type, layer_id_valid, data, data_valid, flit_tx_con, valid_tx_con, ready_tx_con, out_flit[0][2], out_valid[0][2], out_ready[0][2]);
    m_ROM(rst, clk, layer_id, layer_id_type, layer_id_valid, data, data_valid);

    /*m_Core0->rst( rst );
    m_Core0->clk( clk );
    m_Core0->flit_rx( out_flit[0][0] );
    m_Core0->valid_rx( out_valid[0][0] );
    m_Core0->ready_rx( out_ready[0][0] );
    m_Core0->flit_tx( flit_tx[0] );
    m_Core0->valid_tx( valid_tx[0] );
    m_Core0->ready_tx( ready_tx[0] );*/
    ...
    // 3: West router
    m_Router0.in_flit[3]( out_flit[2][1] ); // from router 3, east router
    m_Router0.in_valid[3]( out_valid[2][1]);
    m_Router0.in_ready[3]( out_ready[2][1]);
    // 4: North router
    m_Router0.in_flit[4](  flit_tx_con );
    m_Router0.in_valid[4](  valid_tx_con );
    m_Router0.in_ready[4]( ready_tx_con );
    ...
    // 1: East router
    m_Router2.in_flit[1](  out_flit[0][3] ); // from router 3, west router
    m_Router2.in_valid[1]( out_valid[0][3] );
    m_Router2.in_ready[1]( out_ready[0][3] );
    // 2: South router 
    ...
    // 1: East router
    m_Router3.in_flit[1](   out_flit[0][3] ); 
    m_Router3.in_valid[1]( out_valid[0][3] );
    m_Router3.in_ready[1]( out_ready[0][3] );
    // 2: South router 
